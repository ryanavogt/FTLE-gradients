# This is from the following repository: https://github.com/KyleGoyette/nnRNN
...
from RNN_Cell_2 import OrthoRNNCell, RNNCell
    ...
    if args.net_type == 'nnRNN':
        x = [
            {'params': (param for param in net.parameters()
                        if param is not net.rnn_layer.log_P
                        and param is not net.rnn_layer.P
                        and param is not net.rnn_layer.UppT)},
            {'params': net.rnn_layer.UppT, 'weight_decay': args.Tdecay}
            ]
        y = [
            {'params': (param for param in net.parameters() if param is net.rnn_layer.log_P)}
        ]
    elif args.net_type == 'expRNN':
        x = [
            {'params': (param for param in net.parameters()
                        if param is not net.rnn_layer.log_recurrent_kernel
                        and param is not net.rnn_layer.recurrent_kernel)}
            ]
        y = [
            {'params': (param for param in net.parameters()
                        if param is net.rnn_layer.log_recurrent_kernel)}
        ]
    else:
        x = [
            {'params': (param for param in net.parameters())}
