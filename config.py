...
from dataloader_smnist import *
import lyapunov as lyap
import pickle as pkl
from antisymmetricRNN import *


# from keras.utils import to_categorical

class DataConfig(object):
    def __init__(self, data_dir, batch_size, input_seq_length=1, target_seq_length=1, train_frac=0.8, val_frac=0.1,
                 test_frac=0.1, input_size=1,
                 name_params={}, download=False):
        """
        ...
        if (train_frac + val_frac + test_frac) != 1.0:
            print(
                'Sum of train/val/test split must be 1.0, but sum was {%.1f}. Setting train/val/test split to 0.8/0.1/0.1'.format(
        ...
        self.datasets = create_dataset(self, download=download)
        self.name_params = name_params


class TrainConfig(object):
    def __init__(self, model_dir, batch_size, max_epoch, optimizer, learning_rate, optim_params={}, start_epoch=0,
        ...
        self.optimizer_options = {'adadelta': optim.Adadelta, 'adagrad': optim.Adagrad, 'adam': optim.Adam,
                                  'adamW': optim.AdamW, 'sparseAdam': optim.SparseAdam,
                                  'adamax': optim.Adamax, 'asgd': optim.ASGD, 'rmsProp': optim.RMSprop,
...
class ModelConfig(object):
    def __init__(self, model_type, num_layers, hidden_size, input_size, output_size, dropout, init_type, device,
                 init_params={}, x_init_params={}, other_rnn_params={},
        ...
        self.initializations = {'uni': nn.init.uniform_, 'normal': nn.init.normal_, 'xav_uni': nn.init.xavier_uniform_,
                                'xav_normal': nn.init.xavier_normal_,
                                'ortho': nn.init.orthogonal_, 'kai_uni': nn.init.kaiming_uniform_,
                                'kai_normal': nn.init.kaiming_normal_}
...
class FullConfig(object):
    ...
    def name(self):
        fullname = '{0}_L{1}_H{2}_{3}_drop{4}_{5}_lr{6}_{7}_{8}'.format(self.model.model_type,
                                                                        self.model.rnn_atts['num_layers'],
                                                                        self.model.rnn_atts['hidden_size'],
                                                                        self.model.nonlinearity, self.model.dropout,
                                                                        self.train.optimizer,
                                                                        self.train.learning_rate, self.model.init_type,
                                                                        self.model.init_params[
                                                                            self.model.id_init_param])
        for param, value in self.model.other_rnn_params.items():
            fullname += f'_{param}{value}'
        for param, value in self.data.name_params.items():
            fullname += f'_{param}{value}'
...
class LyapConfig(object):
    ...
    def get_mnist_input(self, fcon):
        ...
        xt = flattened_data[:self.batch_size * self.seq_length * math.floor(
            fcon.data.input_seq_length * len(xt) / (self.batch_size * self.seq_length))]
    ...
    def calc_FTLE(self, le_input, model, fcon, h=None, save_s=False):
        model.eval()
        if h == None:
            h = torch.randn(1, self.batch_size, fcon.model.rnn_atts['hidden_size'])
        ftle_dict = lyap.FTLE(le_input, h, model=model, warmup=0, rec_layer=fcon.model.model_type, save_s=save_s)
        return ftle_dict
